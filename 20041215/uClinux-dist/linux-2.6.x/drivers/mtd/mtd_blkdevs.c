...
int blktrans_romptr(struct block_device *bdev, struct vm_area_struct * vma)
{
	struct mtd_blktrans_dev *dev;
	struct mtd_blktrans_ops *tr;
	...
	tr = dev->tr;
	/* FIXME : I don't know at which level we must << PAGE_SHIFT */
	vma->vm_pgoff <<= PAGE_SHIFT;
	if (!tr->romptr || tr->romptr(dev, vma))
		return -ENOSYS;
	return 0;
}
#endif
