...
#include <linux/mm.h>
static int
mtdblock_romptr(struct mtd_blktrans_dev *dev, struct vm_area_struct * vma)
{
	struct mtd_info *mtd = dev->mtd;
	u_char *ptr;
	size_t len;

	if (!mtd->point /* Can't do it, No function to point to correct addr */

	|| (*mtd->point)(mtd,vma->vm_pgoff,vma->vm_end-vma->vm_start,&len,&ptr) != 0)
		return -ENOSYS;
