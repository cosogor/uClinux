...
/* should be an inline or a macro, but bd_disk`s type is unknown where we use bromptr */
int bromptr(struct block_device *bdev, struct vm_area_struct * vma)
{
	int (*romptr) (struct block_device *, struct vm_area_struct *) =
						bdev->bd_disk->fops->romptr;
	if (romptr)
		return romptr(bdev, vma);
	return -ENOSYS;
}
#endif /* CONFIG_MAGIC_ROM_PTR */
