...
void microblaze_intc_master_enable(void)
{
  __asm__ __volatile__ (
		"	sw	%0, r0, %1;"
		:: "r" (MICROBLAZE_INTC_MASTER_ENABLE_MASK | MICROBLAZE_INTC_HARDWARE_ENABLE_MASK), "r" (MICROBLAZE_INTC_MER_OFFSET+CONFIG_XILINX_INTC_0_BASEADDR));
}
...
int microblaze_intc_get_active_irq(void)
{
...
#if CONFIG_XILINX_INTC_0_HAS_IVR==1
	int irq;
	/* Query the IVR register of the INTC */
	__asm__ __volatile__ ("	lw	%0, r0, %1"
				: "=r" (irq)
				: "r" (MICROBLAZE_INTC_IVR_OFFSET+CONFIG_XILINX_INTC_0_BASEADDR)
				: "memory");
	return irq;
#else
	...
	__asm__ __volatile__ ("	lw	%0, r0, %2;
				...
				: "=r" (isr),
				  "=r" (ier)
				: "r" (MICROBLAZE_INTC_ISR_OFFSET+CONFIG_XILINX_INTC_0_BASEADDR),
				  "r" (MICROBLAZE_INTC_IER_OFFSET+CONFIG_XILINX_INTC_0_BASEADDR)
				: "memory");
...
#endif /* CONFIG_XILINX_INTC_0_HAS_IVR */
}
...
void microblaze_intc_ack_irq(unsigned irq)
{
	...
	__asm__ __volatile__ ("	sw	%0, r0, %1"
				:
				: "r" (irq_mask),
				  "r" (MICROBLAZE_INTC_IAR_OFFSET+CONFIG_XILINX_INTC_0_BASEADDR)
				: "memory");
}
...
void microblaze_intc_enable_irq (unsigned irq)
{
...
#if CONFIG_XILINX_INTC_0_HAS_SIE==1
	/* Use Set Interrupt Enables register (SIE) to avoid test and set */
	__asm__ __volatile__ ("sw	%0, r0, %1"
				: 
				: "r" (irq_mask),
				  "r" (MICROBLAZE_INTC_SIE_OFFSET+CONFIG_XILINX_INTC_0_BASEADDR)
				: "memory");
#else
	...
	__asm__ __volatile__ ("	lw	%0, r0, %3;
				...
				:"0" (old_mask),
				 ...
				 "r" (MICROBLAZE_INTC_IER_OFFSET+CONFIG_XILINX_INTC_0_BASEADDR)
				: "memory");
#endif /* CONFIG_XILINX_INTC_0_HASE_SIE */
}
...
void microblaze_intc_disable_irq (unsigned irq)
{
...
#if CONFIG_XILINX_INTC_0_HAS_CIE==1
	unsigned int irq_mask = 1 << irq;
	__asm__ __volatile__ ("	sw	%0, r0, %1"
				:
				: "r" (irq_mask),
				  "r" (MICROBLAZE_INTC_CIE_OFFSET+CONFIG_XILINX_INTC_0_BASEADDR)
				: "memory");
#else
	unsigned int old_mask;
	unsigned int irq_mask = ~(1 << irq);
	...
	__asm__ __volatile__ ("	lw	%0, r0, %3;
				...
				: "0" (old_mask),
				  ...
				  "r" (MICROBLAZE_INTC_IER_OFFSET+CONFIG_XILINX_INTC_0_BASEADDR)
				: "memory");
#endif /* CONFIG_XILINX_INTC_0_HAS_CIE */
}
