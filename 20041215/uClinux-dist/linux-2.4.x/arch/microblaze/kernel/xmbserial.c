...
{
	if(x=='\n')
		SendByte((unsigned int *)CONFIG_XILINX_UARTLITE_1_BASEADDR, '\r');

	SendByte((unsigned int *)CONFIG_XILINX_UARTLITE_1_BASEADDR,x);
}
...
static struct xmb_serial xmbrs_table[] = {
  { 0, CONFIG_XILINX_UARTLITE_0_BASEADDR, 
  ...
  { 0, CONFIG_XILINX_UARTLITE_3_BASEADDR, 
       CONFIG_XILINX_UARTLITE_3_IRQ,  ASYNC_BOOT_AUTOCONF } /* ttyS3 */
#endif
};

#define	NR_PORTS	(sizeof(xmbrs_table) / sizeof(struct xmb_serial))

/* Reverse lookup table to quickly determine serial info from IRQ number */
static struct xmb_serial* irq_lookup[32];
...
void xmbrs_init_console(void)
{
	...
	volatile unsigned int *uartp= (volatile unsigned int *) CONFIG_XILINX_UARTLITE_0_BASEADDR;
