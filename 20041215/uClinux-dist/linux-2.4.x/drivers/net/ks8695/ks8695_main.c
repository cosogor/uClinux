...
void ks8695_getmac(unsigned char *dst, int index)
{
	...
	if (ep) {
		...
		mp = ep + 0xc000 + (index * ETH_LENGTH_OF_ADDRESS);
		...
		case SIOCDEVPRIVATE + 1:
			...
			switch (data->reg_num)
			{
				case MII_BMCR:
					...
					reg = KS8695_READ_REG(REG_WAN_CONTROL);

					data->val_out |= reg & SW_PORT_FULLDUPLEX
						? BMCR_FULLDPLX : 0;
					data->val_out |= reg & SW_AUTONEGO_RESTART << 16
						? BMCR_ANRESTART : 0;
					data->val_out |= reg & SW_PORT_DISABLE_AUTONEG
						? 0 : BMCR_ANENABLE;
					data->val_out |= reg & SW_PORT_100BASE
						? BMCR_SPEED100 : 0;

					reg = KS8695_READ_REG(REG_WAN_POWERMAGR);

					data->val_out |= reg & POWER_POWERDOWN
						? BMCR_PDOWN : 0;
					
					reg = KS8695_READ_REG(REG_WAN_DMA_TX);
					data->val_out |= reg & DMA_LOOPBACK
						? BMCR_LOOPBACK : 0;
				...
				case MII_BMSR:
					...
					data->val_out |= reg & SW_AUTONEGO_STAT_LINK << 16
						? BMSR_LSTATUS : 0;
					data->val_out |= reg & SW_AUTONEGO_COMPLETE << 16
						? BMSR_ANEGCOMPLETE : 0;
						
					if (reg & SW_PORT_DISABLE_AUTONEG) {
						if (reg & SW_PORT_100BASE)
							if (reg & SW_PORT_FULLDUPLEX)
								data->val_out |= BMSR_100FULL;
							else
								data->val_out |= BMSR_100HALF;
						else
							if (reg & SW_PORT_FULLDUPLEX)
								data->val_out |= BMSR_10FULL;
							else
								data->val_out |= BMSR_10HALF;
					} else
						data->val_out |= BMSR_ANEGCAPABLE | BMSR_10HALF
							| BMSR_10FULL | BMSR_100HALF | BMSR_100FULL;
					break;

				case MII_ADVERTISE:
					/* Advertisement control register */
					data->val_out |= ADVERTISE_CSMA; /* Only mode supported */
				
					reg = KS8695_READ_REG(REG_WAN_CONTROL);

					data->val_out |= reg & SW_AUTONEGO_ADV_10HD << 16
						? ADVERTISE_10HALF : 0;
					data->val_out |= reg & SW_AUTONEGO_ADV_10FD << 16
						? ADVERTISE_10FULL : 0;
					data->val_out |= reg & SW_AUTONEGO_ADV_100HD << 16
						? ADVERTISE_100HALF : 0;
					data->val_out |= reg & SW_AUTONEGO_ADV_100FD << 16
						? ADVERTISE_100FULL : 0;
					data->val_out |= reg & SW_AUTONEGO_PART_10HD << 16
						|| reg & SW_AUTONEGO_PART_10FD << 16
						|| reg & SW_AUTONEGO_PART_100HD << 16
						|| reg & SW_AUTONEGO_PART_100FD << 16
						? ADVERTISE_LPACK : 0;
					break;
				...
				case MII_LPA:
					...
					data->val_out |= reg & SW_AUTONEGO_PART_10HD << 16
						? ADVERTISE_10HALF : 0;
					data->val_out |= reg & SW_AUTONEGO_PART_10FD << 16
						? ADVERTISE_10FULL : 0;
					data->val_out |= reg & SW_AUTONEGO_PART_100HD << 16
						? ADVERTISE_100HALF : 0;
					data->val_out |= reg & SW_AUTONEGO_PART_100FD << 16
						? ADVERTISE_100FULL : 0;
					data->val_out |= reg & SW_AUTONEGO_PART_10HD << 16
						|| reg & SW_AUTONEGO_PART_10FD << 16
						|| reg & SW_AUTONEGO_PART_100HD << 16
			...
			switch (data->reg_num)
			{
				case MII_BMCR:
					if (skipcmd) {
						...
						if (DI.byDisableAutoNego[0] == LINK_SELECTION_FORCED) {
							if ((DI.usCType[0] = ctype) == SW_PHY_AUTO)
								DI.byDisableAutoNego[0] =
									LINK_SELECTION_FULL_AUTO;
							else
								DI.byDisableAutoNego[0] =
									LINK_SELECTION_PARTIAL_AUTO;
						}
					} else if (data->val_in & BMCR_RESET) {
						/* Reset to defaults */
						if ((DI.usCType[0] = SW_PHY_DEFAULT) == SW_PHY_AUTO)
							DI.byDisableAutoNego[0] = LINK_SELECTION_FULL_AUTO;
						else
							DI.byDisableAutoNego[0] =
								LINK_SELECTION_PARTIAL_AUTO;
					} else {
						/* Force link media type */
						if (DI.byDisableAutoNego[0]
							== LINK_SELECTION_PARTIAL_AUTO)
							ctype = DI.usCType[0];	/* Remember advert media */
						DI.byDisableAutoNego[0] = LINK_SELECTION_FORCED;

						if (data->val_in & BMCR_SPEED100) {
							if (data->val_in & BMCR_FULLDPLX)
								DI.usCType[0] = SW_PHY_100BASE_TX_FD;
							else
								DI.usCType[0] = SW_PHY_100BASE_TX;
						} else {
							if (data->val_in & BMCR_FULLDPLX)
								DI.usCType[0] = SW_PHY_10BASE_T_FD;
							else
								DI.usCType[0] = SW_PHY_10BASE_T;
						}
					}
					...
					swConfigureMediaType(Adapter,
						0,	/* WAN */
						DI.usCType[0] == SW_PHY_100BASE_TX ||
						DI.usCType[0] == SW_PHY_100BASE_TX_FD ? 1 : 0,
						DI.usCType[0] == SW_PHY_10BASE_T_FD ||
						DI.usCType[0] == SW_PHY_100BASE_TX_FD ? 1 : 0);
					break;

				case MII_ADVERTISE:
					DI.byDisableAutoNego[0] = LINK_SELECTION_PARTIAL_AUTO;
					/*  
