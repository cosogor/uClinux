...
struct ifreq      *ifr)     /* what interface the request refers to? */
{
	...
	case ETHTOOL_GSTRINGS: {
		...
		pAddr += offsetof(struct ethtool_gstrings, data);
...
struct ethtool_cmd *ecmd) /* mandatory command structure for results */
{
	...
	if (pAC->GIni.GICopperType) {
		...
		ecmd->advertising = (ADV_COPPER_ALL);
...
{
	SK_GEPORT *pPort            = &pAC->GIni.GP[port];
	SK_BOOL    isAutoNegEnabled = SK_TRUE;

	if (pPort->PLinkModeConf == SK_LMODE_HALF || 
	    pPort->PLinkModeConf == SK_LMODE_FULL) {
                isAutoNegEnabled = SK_FALSE;
        }

	epause->autoneg  = isAutoNegEnabled;
	epause->rx_pause = 0;
	epause->tx_pause = 0;
...
struct ethtool_pauseparam *epause) /* pause parameter struct with params */
{
	...
	if (epause->autoneg == AUTONEG_DISABLE) {
		if (pPort->PLinkModeConf == SK_LMODE_AUTOHALF) {
			*Buf = (char) SK_LMODE_HALF;
		}
		if ((pPort->PLinkModeConf == SK_LMODE_AUTOFULL) ||
		    (pPort->PLinkModeConf == SK_LMODE_AUTOBOTH)) {
			*Buf = (char) SK_LMODE_FULL;
		}
	} 
	...
	if (epause->autoneg == AUTONEG_ENABLE) {
		if (pPort->PLinkModeConf == SK_LMODE_HALF) {
			// pPort->PLinkModeConf = SK_LMODE_AUTOHALF;
			*Buf = (char) SK_LMODE_AUTOHALF;
		}
		if (pPort->PLinkModeConf == SK_LMODE_FULL) {
			// pPort->PLinkModeConf = SK_LMODE_AUTOBOTH;
        ...
        if (Ret != SK_PNMI_ERR_OK) {
                printk("ethtool: error changing autoneg parameter\n");
        }

	if(epause->rx_pause && epause->tx_pause) {
		// pPort->PFlowCtrlMode = SK_FLOW_MODE_SYMMETRIC;
		*Buf = (char) SK_FLOW_MODE_SYMMETRIC;
	} else if (epause->rx_pause && !epause->tx_pause) {
		// pPort->PFlowCtrlMode = SK_FLOW_MODE_SYM_OR_REM;
		*Buf = (char) SK_FLOW_MODE_SYM_OR_REM;
	} else if(!epause->rx_pause && epause->tx_pause) {
		// pPort->PFlowCtrlMode = SK_FLOW_MODE_LOC_SEND;
		*Buf = (char) SK_FLOW_MODE_LOC_SEND;
	} else {
		// pPort->PFlowCtrlMode = SK_FLOW_MODE_NONE;
		*Buf = (char) SK_FLOW_MODE_NONE;
        ...
        if (Ret != SK_PNMI_ERR_OK) {
                printk("ethtool: error changing rx/tx pause parameters\n");
        }

	/*
	** It may be that autoneg has been disabled! Therefore set
	** the speed to the previously used value...
	*/
	if (epause->autoneg == AUTONEG_DISABLE) {
		if ((PrevSpeedVal == SK_LSPEED_AUTO) ||
		    (PrevSpeedVal == SK_LSPEED_INDETERMINATED)) {
		...
		*Buf = (char) PrevSpeedVal;
		...
		if (Ret != SK_PNMI_ERR_OK) {
			printk("ethtool: setting the speed parameter\n");
		}
	}
        return 0;
}
...
{
	ewol->supported = 
		WAKE_PHY|WAKE_MAGIC|WAKE_UCAST|WAKE_MCAST|WAKE_BCAST;
...
struct ethtool_value *blinkSecs)  /* how long the LEDs should blink in seconds */
{
	...
	DEV_NET              *pOtherNet = (DEV_NET *) pAC->dev[OtherPort]->priv;
